...
export class UserRegistrationComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  displayedColumns: string[] = ['srno', 'name','userName', 'departmentName', 'officeName', 'subOfficeName','subUserType', 'mobileNo', 'action'];
  ...
  getOffice(deptNo: number) {
    ...
    this.commonService.getOfficeByDeptId(deptNo).subscribe({
      next: (response: any) => {
        ...
        this.changeDepFlag == true ? (this.userFrm.controls['officeId'].setValue(this.commonMethod.checkDataType(this.updatedObj?.officeId) == false ? '' : this.updatedObj?.officeId),this.getSubOffice()) : '';
      },
      error: ((error: any) => { this.error.handelError(error.status) })
    })
  }
  ...
  getSubOffice(){
    let id=this.userFrm.value.officeId;
    if(id!=0){
      this.subOfficeArray = [];
  ...
  onSubmit() {
    ...
    let obj = {
      ...
      "subOfficeId": formData.subOfficeId || 0,
      "isBlock": false,
      "blockDate": new Date(),
      "blockBy": 0,
