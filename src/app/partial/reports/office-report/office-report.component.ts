...
export class OfficeReportComponent implements OnInit {
    ...
    this.apiService.getHttp().subscribe({
      next: (res: any) => {
        if (res.statusCode == 200) {
          this.officeOffReportArray = res.responseData.map((ele:any)=>{ 
  ...
  downloadExcel(){
    let keyValue = this.officeOffReportArray.map((value: any) => Object.keys(value));
  ...
  downloadPdf() {
    let keyPDFHeader = ["Department Name","Office Name", "Received","Pending","Resolved"]; 
    let ValueData = this.officeOffReportArray.reduce(
      (acc: any, obj: any) => [...acc, Object.values(obj).map((value) => value)],
      []
    );// Value Name
